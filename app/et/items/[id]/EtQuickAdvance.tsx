...
import type { StageCode } from "@/lib/et";
import type { StagePatch } from "@/lib/etMutations";

const TODAY = new Date().toISOString().slice(0, 10);

export interface QuickAdvance {
  /** The stage the item is currently sitting at. */
  stage: StageCode;
  stageName: string;
  holder: string | null;
  days: number | null;
  /** True when the current stage is sent but not yet returned (work in progress). */
  inProgress: boolean;
  /** The next unfinished stage to hand the work to (or null if this is the last). */
...
interface Props {
  itemId: string;
  advance: QuickAdvance;
  peopleNames: string[];
}

export default function EtQuickAdvance({ itemId, advance, peopleNames }: Props) {
  ...
  return (
    <div className="mt-3 rounded-xl border border-emerald-200 dark:border-emerald-800/60 bg-emerald-50/40 dark:bg-emerald-900/10 p-3">
      <div className="flex items-center justify-between gap-2">
        ...
        <button
          type="button"
          onClick={() => setOpen((o) => !o)}
          className="btn-press flex-shrink-0 rounded-lg bg-emerald-600 px-3 py-1.5 text-sm font-medium text-white hover:bg-emerald-700"
        >
          {open ? "Cancel" : advance.inProgress ? (advance.nextStage ? "Move to next step" : "Mark done") : "Start this step"}
        </button>
      </div>

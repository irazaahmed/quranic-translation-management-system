...
interface DailyReportContentProps {
  report: DailyReportData | { 
    ...
    languagesWithMeetings: { language: any; meetings: any[] }[] 
  };
}
...
function formatDateRange(start: Date, end: Date): string {
  ...
  if (start.getTime() === end.getTime()) {
    return formatReportDate(start);
  }
  
  return `${startStr} - ${endStr}`;
}

export default function DailyReportContent({ report }: DailyReportContentProps) {
  if (report.totalMeetings === 0) {
    return (
      <div className="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-12 text-center transition-colors duration-200">
        ...
        <h3 className="mt-6 text-xl font-semibold text-gray-900 dark:text-white transition-colors duration-200">No meetings today</h3>
        <p className="mt-2 text-sm text-gray-500 dark:text-gray-400 transition-colors duration-200">
          There are no meetings scheduled for {formatReportDate(report.selectedDate)}.
        </p>
        <a
          href="/meetings/new"
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h2 className="text-lg font-semibold text-gray-900 dark:text-white transition-colors duration-200">
              {formatReportDate(report.selectedDate)}
            </h2>
          ...
          <ReportActions
            title="Daily Meeting Summary"
            periodLabel={formatReportDate(report.selectedDate)}
            totalMeetings={report.totalMeetings}
            totalLanguages={report.totalLanguages}
            languagesWithMeetings={report.languagesWithMeetings as any}
            fileBase="daily-report"
          />
        </div>
      </div>
